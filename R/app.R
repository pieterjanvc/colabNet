...
colabNet <- function() {
  ...
  df = df %>% rowwise() %>%
    mutate(Broad = paste(c(Broad, if(!is.na(Techniques))"Techniques"), collapse = ", "),
           Broad = paste(c(Broad, if(!is.na(Model))"Model"), collapse = ", "))

  #Get the key words by level
  lvl1 = df$MacroArea %>% str_split(", ") %>% unlist() %>% unique()
  lvl2 = df %>% select(-c(MyID:Broad, Extra:Link)) %>% colnames() %>%
    str_replace("_", " ")
  keywords = df %>% select(all_of(str_replace(lvl2, "\\s", "_")))
  lvl3 = keywords %>% unlist() %>% str_split(", ") %>% unlist() %>% unique()
  lvl3 = lvl3[!is.na(lvl3)]

  #Link lvl 3 to lvl 2 keywords (there is no 2 to 1 link)
  lvl2to3 = keywords %>% pivot_longer(everything()) %>%
    filter(!is.na(value), value != "na") %>% mutate(name = str_replace(name, "_", " ") )
  ...
  keywords = bind_rows(
    ...
    lvl2to3 %>% select(keyword = lvl3, parent = lvl2), by = "keyword"
  ) %>% arrange(lvl, keyword)

  #Table with PI info
  PIs = df_net %>% select(piId = MyID, lName = Last_Name, fName = First_Name) %>%
    distinct() %>% left_join(
      df %>% select(piId = MyID, labSite = Link), by = "piId"
    )

  #Link PI to keywords
  piKeywords = apply(df, 1, function(pi){
    bind_rows(
      data.frame(piId = pi[["MyID"]],
  ...
  #List of papers
  papers = df_net %>% select(piId = MyID, title = Title, PMID) %>% distinct()
  ...
  ui <- fluidPage(
    ...
    br(),
    tags$footer(
      p("This app was cretaed to support the Harvard Medical School BBS",
        tags$a("Program in Genetics and Genomics",
               href = "https://projects.iq.harvard.edu/pgg", target= "_blank")),
      p("Content manager: Lorenzo Gesuita -",
  ...
  server <- function(input, output, session) {
    ...
    lvl2Key = reactive({
      ...
      id = piKeywords %>%
        ...
        pull(piId)
      piKeywords %>% filter(lvl == 2, piId %in% id) %>% select(keyword, parent) %>%
    ...
    lvl3Key = reactive({
      ...
      id = piKeywords %>%
        filter(keyword %in% c(lvl1Key[input$lvl1Key_rows_selected,],
                              lvl2Key()[input$lvl2Key_rows_selected,])) %>%
        group_by(piId) %>% filter(n() == 2) %>% ungroup() %>% pull(piId)
      piKeywords %>% filter(lvl == 3, piId %in% id,
    ...
    PIsel = reactive({
      x = c(ifelse(!is.null(input$lvl1Key_rows_selected), lvl1Key[input$lvl1Key_rows_selected,], NA),
      ...
      x = x[!is.na(x)]
      piKeywords %>% filter(keyword %in% x) %>% group_by(piId) %>%
        filter(n() == length(x)) %>% pull(piId)
    })
    ...
    output$piTable = renderDT({
      datatable(PIs %>% filter(piId %in% PIsel()) %>%
    ...
    nodes = df_net %>% select(MyID, First_Name, Last_Name) %>% distinct() %>%
      transmute(id = MyID, label = paste(Last_Name, First_Name),
    ...
    collab = collab %>% select(p1 = MyID, PMID, Title) %>%
      left_join(collab %>% select(p2 = MyID, PMID),
                by = "PMID",relationship = "many-to-many") %>%
      filter(p1 != p2) %>% group_by(PMID) %>% slice(1) %>% ungroup()

    edges = collab %>%
      group_by(p1, p2) %>% summarise(
        nPapers = n(),
        Title = paste(n(), ifelse(n() == 1, "paper", "papers")),
        .groups = "drop")

    edges = edges %>% transmute(
    ...
    observeEvent(input$piTable_rows_selected, {
      piId = PIs %>% filter(piId %in% PIsel()) %>%
        slice(input$piTable_rows_selected) %>% pull(piId)
      authors(piId)
      visNetworkProxy("visGraph") %>%
    ...
    observeEvent(c(input$nodes_selection, input$edges_selection), {
      ...
      if(!any(sel %in% PIsel())){
        ...
        x = PIs %>% filter(piId %in% PIsel()) %>% pull(piId)
    ...
    output$authorTable = renderDT({
      req(authors())
      datatable(nodes %>% filter(id %in% authors()) %>%
                  mutate(Link = sprintf('<a href="%s" target="_blank">%s</a>',
                                        Link, Link)) %>%
    ...
    output$paperTable = renderDT({
      req(authors())
      if(length(authors()) == 1) {
        x = df_net %>% filter(MyID == authors()) %>%
          select(PMID, Title) %>% arrange(Title)
      } else if(length(authors()) == 2){
        x = collab %>%
      ...
      datatable(
        x %>%
          mutate(PMID = sprintf('<a href="%s%s" target="_blank">%s</a>',
