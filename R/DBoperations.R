...
#' @export
#'
dbAddAuthors <- function(authors, conn, commit = T) {
  ...
  authors <- authors |>
    left_join(
      tbl(conn, "authorName") |> select(-default),
  ...
  if (nrow(updated) > 0) {
    ...
    tbl_update(toUpdate, conn, "author", commit = F, returnData = F)
  }
  ...
  if (nrow(new) > 0) {
    ...
    auID <- tbl_insert(auID, conn, "author", commit = F) |> select(auID)
    ...
    # Add author names
    new <- tbl_insert(new, conn, "authorName", commit = F) |>
      mutate(status = "new")
  }
...
#' @export
#'
dbDeleteAuthors <- function(auIDs, conn, commit = T) {
...
#' @export
#'
dbAddMesh <- function(values, type, conn, commit = T) {
  ...
  if (length(toAdd) > 0) {
    ...
    tbl_insert(meshTerms, conn, "meshTerm", commit = F, returnData = F)
    ...
    tbl_insert(meshTree, conn, "meshTree", commit = F, returnData = F)
  }
...
#' @export
#'
dbFlagUpdate <- function(action, conn, commit = T) {
  action <- data.frame(
  ...
  uID <- tbl_insert(action, conn, "updateData", commit = commit) |> pull(uID)
...
dbAddAuthorPublications <- function(
  ...
  if (all(arInfo$status == "existing")) {
    ...
    tbl_insert(action, conn, "updateData", commit = F, returnData = F)
  ...
  if (nrow(meshDescriptors) > 0) {
    ...
    if (length(meshui) > 0) {
      result <- dbAddMesh(meshui, "meshui", conn = conn, commit = F)
    }
  ...
  if (flagUpdate) {
    . <- dbFlagUpdate(1, conn = conn, commit = F)
  }
...
#' @export
#'
dbDeleteArticle <- function(arIDs, conn, commit = T) {
  # Get all articles and authors to remove
  toRemove <- tbl(conn, "coAuthor") |>
  ...
  removeAuth |>
    ...
    select(auID) |>
    unique() |>
    tbl_delete(conn, "author", commit = F, returnData = F)
  ...
  removeAuth |>
    ...
    seect(anID) |>
    unique() |>
    tbl_delete(conn, "authorName", commit = F, returnData = F)

  # Delete articles
  tbl_delete(
    data.frame(arID = arIDs),
    conn,
    "article",
    commit = F,
    returnData = F
