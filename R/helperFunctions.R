...
#' @param includeMissing (Default = FALSE) Include results with missing
#' author affiliations
#'
...
tempFileCheck <- function(folder, hours, totalSize, pattern = NULL) {
  ...
  fileInfo <- fileInfo |>
    ...
    mutate(
      delta = difftime(Sys.time(), .data$mtime, units = "hours") |>
        as.numeric(),
      expired = .data$delta > .data$hours,
      cumSum = cumsum(ifelse(.data$expired, 0, .data$size)),
      exceedCapacity = .data$cumSum / 1024^2 > .data$totalSize,
      toRemove = .data$expired | .data$exceedCapacity,
  ...
  summary <- fileInfo |>
    ...
    summarise(
      n = n(),
      totalSize = sum(.data$size) / 1024^2,
      nExpired = sum(.data$expired),
      nExceedCapacity = sum(.data$exceedCapacity & !.data$expired)
    )
