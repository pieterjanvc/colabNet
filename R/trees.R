...
#' @param conn (optional if dbSetup() has been run)
...
#' @export
#'
dbTreeFromMesh <- function(uids, roots, conn) {
...
#'
#' @export
dbPaperMesh <- function(auIDs, roots, conn) {
...
#'
#' @export
dbMeshTree <- function(papermesh, roots, conn) {
  ...
  tree <- dbTreeFromMesh(papermesh$uid, roots = roots, conn = conn)
